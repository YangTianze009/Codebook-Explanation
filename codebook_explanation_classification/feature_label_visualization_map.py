...
def generate_max_activation_image(model, target_label, device, input_shape=(1, 256, 16, 16), lr=0.01, num_steps=10000, reg=0.01):
    # 初始化输入图像
    ...
    for step in tqdm(range(num_steps), desc="Optimizing image"):
        ...
        loss = -output[0, target_label] + reg * torch.mean(input_embedding ** 2)  # 负的激活值，因为我们要最大化
...
def main(args):
    ...
    classification_model_path = f"/data/ty45972/taming-transformers/codebook_explanation/checkpoints/ClassificationNet{args.model}/best_model.pth"
    classification_model = load_classification_model(args.model, classification_model_path, device)
    
    # 生成最大化目标类别激活值的图像
    target_label = args.label_index
    optimized_image_embedding = generate_max_activation_image(classification_model, target_label, device)
    
    # 保存生成的input embedding
    ...
    output_path = feature_visualization_map_path + f"optimized_image_embedding.npy"
    np.save(output_path, optimized_image_embedding.detach().cpu().numpy())
    ...
    quantized_optimized_embedding, token_combination, token_feature_maps, original_feature_maps = decode_input_embedding(optimized_image_embedding, VQ_model)
    for token_feature_map in token_feature_maps:
        token_feature_map = chw_to_pillow(token_feature_map)
        token_feature_map.save(feature_visualization_map_path + "token_feature_map.png")

    for original_feature_map in original_feature_maps:
        original_feature_map = chw_to_pillow(original_feature_map)
        original_feature_map.save(feature_visualization_map_path + "original_feature_map.png")
