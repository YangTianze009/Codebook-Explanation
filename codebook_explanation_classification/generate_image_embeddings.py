...
# 解析命令行参数
parser = argparse.ArgumentParser(description="Generate Embeddings for Image Classification")
...
# 设置输出路径
OUTPUT_PATH = f"/data/ty45972/taming-transformers/image_classifcation/datasets/VQGAN_{args.token_num}"
...
def process_and_save_embeddings(csv_file, output_dir, output_csv):
    output_csv_path = f"/data/ty45972/taming-transformers/image_classifcation/datasets/VQGAN_{args.token_num}/" + output_csv
    ...
    with open(output_csv_path, mode='w', newline='') as file:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df)):
            ...
            try:
                ...
                embedding_filename = os.path.basename(img_path).replace('.JPEG', '.npy')
...
if __name__ == "__main__":
    ...
    TRAIN_CSV = f"/data/ty45972/taming-transformers/image_classifcation/datasets/training_images.csv"
    VAL_CSV = f"/data/ty45972/taming-transformers/image_classifcation/datasets/validation_images.csv"
    TEST_CSV = f"/data/ty45972/taming-transformers/image_classifcation/datasets/testing_images.csv"

    process_and_save_embeddings(TRAIN_CSV, os.path.join(OUTPUT_PATH, "train"), "train_embeddings.csv")
    process_and_save_embeddings(VAL_CSV, os.path.join(OUTPUT_PATH, "validation"), "validation_embeddings.csv")
