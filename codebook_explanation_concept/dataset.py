...
class EmbeddingDataset(Dataset):
    def __init__(self, embeddings_file, data_folder):
        ...
        with open(embeddings_file, 'r') as f:
            ...
            next(reader)  # 跳过标题行
            for row in reader:
                filename = row[0]  # 读取文件名
                label = ast.literal_eval(row[1])  # 解析标签字符串为列表
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(embedding, dtype=torch.float32), torch.tensor(label, dtype=torch.float32)  # 返回 PyTorch 张量形式的嵌入向量和标签

def get_train_val_dataloaders(train_csv, val_csv, data_folder, batch_size):
    train_dataset = EmbeddingDataset(f"{data_folder}/" + train_csv, f"{data_folder}/train")  # 创建训练集数据集实例
    val_dataset = EmbeddingDataset(f"{data_folder}/" + val_csv, f"{data_folder}/validation")  # 创建验证集数据集实例

    # 创建数据加载器
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)  # 训练集数据加载器，支持打乱数据
