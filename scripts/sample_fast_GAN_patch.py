...
def extract_and_save_patches(logs, index_sample, save_dir, base_count, patch_size=16, image_size=256):
    """
    Extract and save patches from each image corresponding to the indices, and save them in the respective folders.

    Args:
        logs (dict): Dictionary containing the sampled images and their corresponding conditional labels.
        index_sample (torch.Tensor): Sampled indices with shape (batch_size, num_indices).
        save_dir (str): Directory to save the extracted patches.
        base_count (int): Base count for naming the saved images.
        patch_size (int): Size of the patches in the feature map, default is 16.
        image_size (int): Original image size, default is 256.
    """
    images = logs["samples"]
    condlabel = logs["class_label"]
    
    # Ensure the save directory exists
    os.makedirs(save_dir, exist_ok=True)
    
    batch_size, num_indices = index_sample.shape
    _, _, img_height, img_width = images.shape
    
    # Ensure the image size is correct
    assert img_height == image_size and img_width == image_size, "Image size does not match"
    
    scale_factor = image_size // patch_size  # Downsampling scale factor
    
    # Create a dictionary to keep track of the current count for each index
    index_count = {}
    
    for batch_idx in range(batch_size):
        img = images[batch_idx].cpu().numpy()
        img = (255 * ((img + 1) / 2)).astype(np.uint8).transpose(1, 2, 0)  # Convert to 0-255 range and HWC format
        cond_label_value = condlabel[batch_idx].item() if isinstance(condlabel, torch.Tensor) else condlabel
        
        for idx in range(num_indices):
            index = index_sample[batch_idx, idx].item()
            
            # Calculate the position in the 16x16 feature map
            feature_map_row = idx // patch_size
            feature_map_col = idx % patch_size
            
            # Map back to the 256x256 image position
            original_row_start = feature_map_row * scale_factor
            original_col_start = feature_map_col * scale_factor
            original_row_end = original_row_start + scale_factor
            original_col_end = original_col_start + scale_factor
            
            # Crop the patch
            patch = img[original_row_start:original_row_end, original_col_start:original_col_end]
            
            # Save the patch
            patch_image = Image.fromarray(patch)
            patch_save_dir = os.path.join(save_dir, str(index))
            os.makedirs(patch_save_dir, exist_ok=True)
            
            # Determine the file name based on the current count for this index
            if index not in index_count:
                index_count[index] = 0
            index_count[index] += 1
            patch_number = index_count[index]
            patch_save_path = os.path.join(patch_save_dir, f"{cond_label_value}_{base_count}_{patch_number}.png")
...
def sample_classconditional(model, batch_size, class_label, steps=256, temperature=None, top_k=None, callback=None,
    ...
    # print(f"index_sample is {index_sample.shape}")
    if verbose_time:
    ...
    if not unconditional:
        ...
        for class_label in tqdm(given_classes, desc="Classes"):
            for n, bs in tqdm(enumerate(batches), desc="Sampling Class"):
                if bs == 0: break
                logs, index_sample = sample_classconditional(model, batch_size=bs, class_label=class_label,
                                               temperature=temperature, top_k=top_k, top_p=top_p)
                save_from_logs(logs, logdir, base_count=n * batch_size, cond_key=logs["class_label"])
                patch_img_save_dir = "./patches"
                extract_and_save_patches(logs, index_sample, save_dir=patch_img_save_dir, base_count=n * batch_size)
    else:       
