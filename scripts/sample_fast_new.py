...
def sample_classconditional(model, batch_size, class_label, steps=256, temperature=None, top_k=None, callback=None,
    ...
    return log, quant_z, index_sample
...
def run(logdir, model, batch_size, temperature, top_k, unconditional=True, num_samples=50000,
    ...
    embedding_save_path = "/data/ty45972/taming-transformers/codebook_explanation/datasets/VQGAN_16384_generated/"
    os.makedirs(embedding_save_path, exist_ok=True)
    # 创建CSV文件并写入表头
    csv_file_path = os.path.join(embedding_save_path, "embedding_info.csv")
    ...
    if not unconditional:
        ...
        for class_label in tqdm(given_classes, desc="Classes"):
            for n, bs in tqdm(enumerate(batches), desc="Sampling Class"):
                if bs == 0: break
                logs, quant_z, index = sample_classconditional(model, batch_size=bs, class_label=class_label,
                                               temperature=temperature, top_k=top_k, top_p=top_p)
                save_from_logs(logs, logdir, base_count=n * batch_size, cond_key=logs["class_label"], quant_z=quant_z, index=index, csv_file_path=csv_file_path)
    else:
        ...
        for n, bs in tqdm(enumerate(batches), desc="Sampling"):
            ...
            save_from_logs(logs, logdir, base_count=n * batch_size, quant_z=quant_z, index=index, csv_file_path=csv_file_path)


def save_from_logs(logs, logdir, base_count, key="samples", cond_key=None, quant_z=None, index=None, csv_file_path=None):
    embedding_save_path = "/data/ty45972/taming-transformers/codebook_explanation/datasets/VQGAN_16384_generated/"
    xx = logs[key]
    for i, x in enumerate(xx):
        x = chw_to_pillow(x)
        count = base_count + i
        label = cond_key[i].item()
        os.makedirs(os.path.join(embedding_save_path, str(label)), exist_ok=True)
        quant_z_save_path = os.path.join(embedding_save_path, str(label), f"{count:06}.npy")
        np.save(quant_z_save_path, quant_z[i].cpu().numpy())  # 保存quant_z为.npy格式

        if csv_file_path is not None:
            with open(csv_file_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([f"{label}/{count:06}.npy", label, index[i].cpu().numpy().tolist()])  # 写入文件路径, label和index
